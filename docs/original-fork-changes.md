...
改动后，平台获得了覆盖当前 113 项公开名称的工具层：12 项继续由专用环境负责，新增
`EWToolSpace` 提供其余 101 项；工具拥有可恢复状态、权限与容量约束，并进入 replay 和
...
1. 113 项公开工具由 101 项通用工具和 12 项专用工具组成。
2. 只启用 navigation + memory 时，模型工具面从 101 项缩减到 17 项。
